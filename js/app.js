...
if (hideBtnSolutions && solutions) {
  hideBtnSolutions.addEventListener('click', () => {
  ...
  if (error == 0) {
    ...
    for (const checkImput of checkImputs) {
      if (checkImput.checked) {
        form.insertAdjacentHTML('beforeend', '<input type="hidden" name="check[]" value="' + checkImput.value + '" class="check-hidden">');
      }
    }
    ...
    if (ajax) {
      ...
      if (response.ok) {
        ...
        if (popupText) {
          popupText.textContent = 'Ваше сообщение успешно отправлено';
        }
        popup_open(message);
        form_clean(form);
      } else {
        if (popupText) {
          popupText.textContent = 'Ошибка при отправке данных';
          popup_open(message);
        }
        form.classList.remove('_sending');
      }
...
function form_clean(form) {
  ...
  let hiddenInputs = form.querySelectorAll('.check-hidden');
